...
parameters: DocumentIdParameter.extend({
textQuery: z.string().min(1).optional().describe('Exact text to locate. Returns one result per occurrence with its document index range.'),
elementType: z.enum(['paragraph', 'table', 'list', 'image']).optional().describe('List structural elements of this type. Only "paragraph" and "table" are supported; "list" and "image" are rejected.'),
}),
execute: async (args, { log }) => {
const docs = await getDocsClient();
