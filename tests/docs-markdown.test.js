...
import { convertDocsJsonToMarkdown } from '../dist/googleDocsApiHelpers.js';
...
describe('readGoogleDoc markdown wiring in server.ts', () => {
  it('calls GDocsHelpers.convertDocsJsonToMarkdown and passes documentTab.lists', () => {
    const source = readFileSync(join(srcDir, 'server.ts'), 'utf8');
    assert.ok(source.includes('GDocsHelpers.convertDocsJsonToMarkdown'));
    assert.ok(source.includes('documentTab.lists'));
